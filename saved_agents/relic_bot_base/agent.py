...
class Ship:
    ...
    def __repr__(self):
        return (
            f"Ship({self.unit_id}, starting_position={self.starting_position}, node={self.node.coordinates}, energy={self.energy},)"
        )
    ...
    def create_actions_array(self):
        ...
        for i, ship in enumerate(ships):
            if ship.action is not None:
                if ship.action == ActionType.center and len(ship.sap_targets) > 0 and ship.energy > Global.UNIT_SAP_COST:
